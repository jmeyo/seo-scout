class HtmlReporter {
  async generate(results, options = {}) {
    ...
    const clientReport = Boolean(options.clientReport);
    const detailedPages = summaryMode ? [] : pages.slice(0, maxPages);
    const hasTruncatedDetails = !summaryMode && pages.length > detailedPages.length;
...
<body>
  <div class="container">
    ...
    ${clientReport ? this.renderClientSummary(results) : ''}

    ${this.renderHighlights(pages, maxPages)}
    ...
    <div class="pages">
      <h2>Summary Mode</h2>
      <div class="info-box">
        Summary mode is enabled. Detailed page cards are hidden.
      </div>
    </div>
    ` : `
    <div class="pages">
      <h2>Page Details (${detailedPages.length}/${pages.length})</h2>
      ${hasTruncatedDetails ? `
      <div class="info-box">
        Showing the first ${detailedPages.length} pages. Use <code>--max-pages</code> to change the limit.
      </div>
      ` : ''}
      ${detailedPages.map(page => this.renderPage(page)).join('')}
    </div>
    `}
  ...
  renderClientSummary(results) {
    ...
    const seoCoverage = summary.totalPages > 0
      ? Math.round((summary.withDescription / summary.totalPages) * 100)
      : 0;
    const socialCoverage = summary.totalPages > 0
      ? Math.round((summary.withOpenGraph / summary.totalPages) * 100)
      : 0;

    const statusLabel = this.getClientStatusLabel(summary, errors.length);
    const actions = this.getClientActions(summary, errors.length);

    return `
    <div class="client-box">
      <h2>Client Summary</h2>
      <p><strong>Overall SEO status:</strong> ${statusLabel}</p>
      <p><strong>Key indicators:</strong> ${seoCoverage}% pages with meta descriptions, ${socialCoverage}% pages with Open Graph tags, ${errors.length} technical page errors.</p>
      <p><strong>Recommended next actions:</strong></p>
      <ul>
        ${actions.map(action => `<li>${this.escape(action)}</li>`).join('')}
      </ul>
    </div>
    `;
  }

  getClientStatusLabel(summary, errorCount) {
    if (errorCount > 0) {
      return 'Needs action';
    }

    const descriptionRatio = summary.totalPages > 0 ? summary.withDescription / summary.totalPages : 0;
    if (descriptionRatio >= 0.9 && summary.duplicateDescriptions.length === 0) {
      return 'Healthy baseline';
    }

    if (descriptionRatio >= 0.7) {
      return 'Good, with improvements needed';
    }

    return 'Needs improvement';
  }

  getClientActions(summary, errorCount) {
    const actions = [];

    if (errorCount > 0) {
      actions.push(`Fix the ${errorCount} pages returning server/client errors first.`);
    }

    if (summary.duplicateDescriptions.length > 0) {
      actions.push(`Rewrite duplicate meta descriptions on ${summary.duplicateDescriptions.length} duplicated groups.`);
    }

    const missingDescriptions = summary.totalPages - summary.withDescription;
    if (missingDescriptions > 0) {
      actions.push(`Add meta descriptions on ${missingDescriptions} pages that are missing one.`);
    }

    const missingCanonical = summary.totalPages - summary.withCanonical;
    if (missingCanonical > 0) {
      actions.push(`Add canonical tags on ${missingCanonical} pages to reduce duplicate indexing risks.`);
    }

    if (actions.length === 0) {
      actions.push('Keep current SEO setup and monitor Search Console performance weekly.');
    }

    return actions.slice(0, 5);
  }

  renderHighlights(pages, maxItems) {
    const normalizedMax = Number.isInteger(maxItems) && maxItems > 0 ? maxItems : 30;
    const pagesWithIssues = pages
      .map(page => {
        ...
        return {
          ...
          score: checks.errors.length * 10 + checks.warnings.length
        };
      })
      .filter(item => item.score > 0)
      .sort((a, b) => b.score - a.score)
      .slice(0, Math.min(normalizedMax, 40));
    ...
    return `
    <div class="pages">
      <h2>Top Pages Needing Attention</h2>
      <table>
        <thead>
          <tr>
            ...
            <th style="width: 120px;">Warnings</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr>
            <td>${index + 1}</td>
            <td style="word-break: break-all;">${this.escape(page.url)}</td>
            <td><span class="badge ${page.errors > 0 ? 'badge-error' : 'badge-success'}">${page.errors}</span></td>
            <td><span class="badge ${page.warnings > 0 ? 'badge-warning' : 'badge-success'}">${page.warnings}</span></td>
          </tr>
          `).join('')}
        </tbody>
      </table>
